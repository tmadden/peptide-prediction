...
class SimpleNeuralNet(pace.PredictionAlgorithm):
    ...
    def define_architecture(self, allele_set_size):
        ...
        L8 = RSDLayer(aaDim*8, 9, activation='relu', name='L8')(my_input)

        # assume three input blocks
        # and a total neuron count of 8 (4 and 4) for the output
        L7 = RSDLayer(2*7, 8, activation='relu', name='regionalInteractions')(L8)

        # a fully connected layer for all interaction terms. making this size 5 for now.
        cf = Dense(5, activation='relu', name='denseConnections')(L7)
...
scores, all_fold_results = pace.evaluate(SimpleNeuralNet,
                    selected_lengths=[9],selected_alleles=b35, dataset=pace.data.load_dataset(16), nbr_train=10, nbr_test=10)
