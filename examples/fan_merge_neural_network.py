...
class FanMergeAlgorithm(pace.PredictionAlgorithm):
    ...
    def train(self, hits, misses):
        ...
        # also, get this "16" out of there. pass in via init or something.
    ...
    def define_architecture(self, allele_set_size):
        ...
        for i in range(len(pepLengths)):
            thename = 'pep' + str(i + 8)
            inputPep.append(
                Input(shape=(pepLengths[i] * aaDim,), dtype='float32', name=thename))

        # now send each of these through some RSDLayers.

        # inb = input number of blocks
        # ibs = input block size
        # for amino acids coded with 5D encoding, and a string of 9 of them,
        # inb=9 and ibs=5
        # obs = output block size, which allows us to take the 5D and shrink it if desired.
        # inb=3
        # ibs=4

        obs = 2

        # input = Input(shape=(inb*ibs,), dtype='float32', name='i12')
        # x = RSDLayer(obs*(inb-1), inb, activation='relu')(input)
        # x = Dense(3, activation='relu')(x)

        # fancy way will be something like this. but for now i'll hard code
        # for i in range(len(pepLengths)):
        #    for j in range(pepLengths[i]-1,6,-1):
        #        thename='pep'+str(i+8)+'j'+str(j)
        #        print(thename)

        # take the 11 mers down to 7:
        # note RSDLayer(output length, input number of blocks)
        x11 = RSDLayer(obs * 10, 11, activation='relu')(inputPep[3])
        x11 = RSDLayer(obs * 9, 10, activation='relu')(x11)
        x11 = RSDLayer(obs * 8, 9, activation='relu')(x11)
        x11 = RSDLayer(obs * 7, 8, activation='relu')(x11)

        x10 = RSDLayer(obs * 9, 10, activation='relu')(inputPep[2])
        x10 = RSDLayer(obs * 8, 9, activation='relu')(x10)
        x10 = RSDLayer(obs * 7, 8, activation='relu')(x10)

        x9 = RSDLayer(obs * 8, 9, activation='relu')(inputPep[1])
        x9 = RSDLayer(obs * 7, 8, activation='relu')(x9)

        x8 = RSDLayer(obs * 7, 8, activation='relu')(inputPep[0])

        # now feed them all into a shared 7 layer
        # by adding them together:

        added = keras.layers.Add()([x8, x9, x10, x11])

        shared76 = RSDLayer(obs * 6, 7, activation='relu', name='pepShared76')(added)
